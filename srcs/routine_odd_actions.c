...
/*   Updated: 2024/05/20 17:14:07 by crea             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_first_meal(t_table *table, t_philo *current_philo)
{
	...
	if (current_philo->first_meal && 
		(current_philo->index % 2 == 0 || current_philo->index == table->nbr_of_philo))
	{
...
void	thinking_odd(t_table *table, t_philo *current_philo)
{
	...
	if (current_philo->index == table->nbr_of_philo)
		usleep((current_philo->time_to_eat * 2) * 1000);
	else
		usleep(current_philo->time_to_eat * 1000);
